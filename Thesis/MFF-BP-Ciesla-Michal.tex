\documentclass[final]{./MFFPrace}

\usepackage[starfontserif]{starfont}
\usepackage{caption}
\usepackage{subfigure}
\usepackage{textcomp,scalerel}
\usepackage[outdir=./img/]{epstopdf}

\NazevPrace{Identifikace meteorických rojů}
\NazevPraceEN{Meteor Shower Identification}
\AutorPrace{Michal~Ciesla}
\RokOdevzdani{2024}
\Katedra{Astronomický ústav AV ČR}
\KatedraEN{Astronomical~Institute, Academy of Sciences of the Czech Republic}
\TypPracoviste{Ústav}
\TypPracovisteEN{Institute}
\Vedouci{RNDr.~Pavel~Koten,~Ph.D.}
\KatedraVedouciho{\IKatedra}
\KatedraVedoucihoEN{\IKatedraEN}
\StudijniProgram{Fyzika}
\StudijniObor{FP}
\Podekovani{Děkuji vedoucímu bakalářské práce, RNDr.~Pavlu~Kotenovi,~Ph.~D., za účinnou metodickou a odbornou pomoc, vstřícnost a pozitivní přístup, díky kterým probíhalo zpracování bakalářské práce hladce a efektivně. Děkuji také za ochotné a bohaté představení praktických aspektů meteorické astronomie, které značně napomohlo v představě o reálných měřeních a přineslo s sebou také vlnu inspirace.}
\Abstrakt{\input{kapitoly/abstrakt}}
\AbstraktEN{\input{kapitoly/abstrakt-en}}
\KlicovaSlova{\raggedright {meteory}, {meteorické roje}, {dráhy meteoroidů}, {elementy dráhy}, {$D$-kritérium}, {míra orbitální odlišnosti}}
\KlicovaSlovaEN{\raggedright {meteors}, {meteor swarms}, {meteoroid orbits}, {orbital elements}, {$D$-criterion}, {orbital dissimilarity measure}}

\addbibresource{literatura.bib}

\captionsetup{justification=centering}

\def\eCheck{\scalerel*{\includegraphics{img/emoji/check_mark_flat.eps}}{\textrm{\textbigcircle}}}
\def\eCross{\scalerel*{\includegraphics{img/emoji/multiply_flat.eps}}{\textrm{\textbigcircle}}}

\begin{document}
    \maketitle
    \tableofcontents
    \begin{todolist}% MARK: To-Do list
        \pagebreak
        \item[\done] Úvod
        \item[\done] Observace, měření a reprezentace
        \begin{todolist}
            \item[\done] Elementy dráhy
            \item[\done] Fotografická měření
        \end{todolist}
        \item[\done] Historické metody klasifikace
        \begin{todolist}
            \item[\done] Přímé porovnávání elementů dráhy
            \item[\done] Funkce orbitální odlišnosti, $D$-kritérium
        \end{todolist}
        \item[\done] Revidované heliocentrické metody
        \begin{todolist}
            \item[\done] Revidovaná funkce odlišnosti $D_\text{D}$ a příslušnost kometám
            \item[\done] Hybridní funkce $D_\text{H}$
        \end{todolist}
        \item[\done] Geocentrický přístup
        \begin{todolist}
            \item[\done] Semi-invariantní orbitální veličiny
            \item[\done] Geocentrická funkce $D_\text{N}$
        \end{todolist}
        \item[\done] Porovnání úspěšnosti metod klasifikace
        \item[\done] Praktická část
        \item Závěr
    \end{todolist}
    \noindent
    
    \include{kapitoly/uvod}
    \include{kapitoly/observace-mereni-a-reprezentace}
    \include{kapitoly/historicke-metody-klasifikace}
    \include{kapitoly/revidovane-heliocentricke-metody}
    \include{kapitoly/geocentricke-metody}
    \include{kapitoly/porovnani-uspesnosti-metod-klasifikace}
    \include{kapitoly/prakticka-cast}
    \include{kapitoly/zaver}

    \printbibliography[title=Seznam použité literatury]% MARK: Seznam použité literatury
    \addcontentsline{toc}{chapter}{Seznam použité literatury}

    \noindent
    \note{\textbf{Statistics of Meteor Streams} \cite{dsh}: míra orbitální odlišnosti $D(A,B)$, odlišnost od průměru $D(M,N)$, zanedbatelnost $\nu$, závislost orbitálních proměnných, odhad náhodných přiřazení orbitů do rojů, klasifikace rojů (major/minor/spurious), tabulky orbitálních parametrů, náležitost rojů ke kometám, perturbace}\\
    \note{\textbf{A Southern Hemisphere Radio Survey...} \cite{radiosurvey}: technika měření, podmínky seskupování orbitů do rojů, vyřazení malých skupin, podobnost ``minor'' a ``major'' rojů, dvojí detekce rojů s nízkou inklinací}\\
    \note{\textbf{Remarks...} \cite{remarks}: příklad fiktivních orbitů protínající oběžnou dráhu Země, úměrnost $D_\text{SH}$ rychlosti, Drummondova funkce odlišnosti, nežádoucí jevy $D_\text{SH}$ a $D_\text{D}$, rozbor oblouků vs. tětiv ve vzorcích, hybridní funkce odlišnosti $D_\text{H}$}\\
    \note{\textbf{...a new approach} \cite{newapproach}: Keplerovské souřadnice, lineární závislost Keplerovských souřadnic v $D_\text{SH}$, geocentrický přístup, semi-invariantní veličiny, geocentrická funkce odlišnosti $D_\text{N}$}\\
    \note{\textbf{Performance of the $D$-criteria...} \cite{galligan}: hranice hodnot odlišnosti pro různé inklinace, sporadické pozadí, porovnání účinnosti metod}\\
    \note{\textbf{...Comet and Meteor Shower Association} \cite{cometassoc}: příslušnost některých meteorických rojů kometám, tabulka elementů dráhy těchto komet, poznámky k $D_\text{SH}$, lepší rozpis $D_\text{D}$}\\
    \note{\textbf{...Photographic Fireball Networks} \cite{ceplecha}: metody fotografického záznamu meteorů, definice radiantu, výpočet orbitu, Gaussova gravitační konstanta $k=0{,}01720209895$}\\
    \note{\textbf{Definitions...in meteor astronomy} \cite{meteorastro}: definice pojmů meteorit, meteoroid a meteor}\\

    \listoffigures
    \listoftables
    \chapwithtoc{Seznam elektronických příloh}
    V elektronických přílohách této práce (a na USB Flash disku přiloženém k tištěné verzi práce) se nacházejí následující položky:
    \begin{itemize}
        \item Soubor \texttt{MFF-BP-Ciesla-Michal.pdf}: Elektronická verze této práce
        \item Adresář \texttt{MeteorShowerIdentificationConsole/}: Zdrojový kód praktické části
        \item Soubor \texttt{meteors.exe}: Spustitelná verze praktické části pro operační systémy Windows
    \end{itemize}
    \noindent
    Zdrojový kód praktické části je také dostupný v repositáři\\\href{https://github.com/Akimayo/MeteorShowerIdentification}{https://github.com/Akimayo/MeteorShowerIdentification}.
\end{document}